...
from datasets.datasources import Datasource, Semantics, DatasourceColumn
...
class DatasetConfig:
    ...
    def from_dict(cls, env):      
        parse_column_flags = \
            lambda kv: (kv[0], DatasourceColumn.from_dict(kv[1]))   if kv[0] == "include_columns" else \
                       (kv) 
    ...
    def __init__(self, dataset_json: dict[str, Any]): # type: ignore
        ...
        for source_name, source_json in self.conf.datasources.items():
            ...
            if SourceClass is not None:
                source = SourceClass(source_json=source_json)
