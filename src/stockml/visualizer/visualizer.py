...
from stockml.datasets.Common import MultisourceTimeSeriesDataset
...
class VApp(QtWidgets.QMainWindow):
    ...
    def update_iter(self, iter):
        self.iter_label.setText(f"Iteration {iter}")
    ...
    def update_price_graph(self, x, y, y_hat):
        target_idx = self.dataset.columns.index(self.dataset.target)
        # print(close_idx)
        # raise "x"
        ...
        if x is not self.price_data:
            ...
            if "close_macd26" in self.dataset.columns:
                macd_data = x[0, -50:, self.dataset.columns.index("close_macd26")]
        ...
        if y_hat is not self.price_pred_data and self.price_data is not None:
            ...
            close_diff = np.sign(np.abs(np.sign(np.diff(y_data)) - np.sign(np.diff(y_hat_data))))
...
def visualize_module(module: torch.nn.Module):
    fields = inspect.getmembers(module, lambda x: isinstance(x, torch.nn.Module))
