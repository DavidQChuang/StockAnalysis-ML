...
from stockml.datasets.sources.columns import IndicatorColumn, Semantics, SourceColumn, generate_indicators
...
class DatasourceConfig:
    ...
    def parse_columns(cls):
        return (
            lambda kv: (kv[0], Semantics.parse(kv[1]))
            if kv[0] == "column_flags"
            else (kv[0], Source.parse_interval(kv[1]))
            if kv[0] == "interval"
            else (kv[0], [SourceColumn.from_dict(col_info) for col_info in kv[1]])
            if kv[0] == "include_columns"
            else (kv[0], [IndicatorColumn.from_dict(col_info) for col_info in kv[1]])
            if kv[0] == "indicators"
            else (kv)
        )
    ...
    def from_dict(cls, env):
        return cls(**{k: v for k, v in map(cls.parse_columns(), env.items()) if k in inspect.signature(cls).parameters})  # type: ignore
    ...
    def should_generate_intervals(self):
        has_resamplable_data = ((Semantics.OHLC in self.column_flags) or (Semantics.BIDASK in self.column_flags)) and (
    ...
    def get_default_aggregator(self):
        if Semantics.OHLCV in self.column_flags:
            aggregator = {"open": "first", "high": "max", "low": "min", "close": "last", "volume": "sum"}
    ...
    def is_multi_source(self):
        """If the Datasource has multiple types of data (OHLC, BIDASK, NEWS) at once, it will be split into multiple datasources."""  # noqa: E501
    ...
    def has_weird_flags(self):
        """If the Datasource has weird flags, e.g. missing a part of OHLC or BIDASK or having no TIMESTAMP, gives a warning."""  # noqa: E501
        warnings = []
...
class Source(ABC):
    def __init__(self, source_json: dict[str, dict] = None, config=None, force_overwrite=False):
    ...
    def parse_interval(cls, interval_str: str) -> int | timedelta | None:
        ...
        match = re.match(
            r"([0-9]+)(t|tick|ticks|sec|second|seconds|min|minute|minutes|hr|hour|hours|d|day|days|w|week|weeks)",
            interval_str,
        )
        if match is None:
        ...
        unit = match.group(2)
        unit = cls.unit_map.get(unit, unit)
    ...
    def _retrieve_dataframe(self, source_json: dict, config: DatasourceConfig, force_overwrite=False) -> pd.DataFrame:
        """
        Retrieve data from the data source and parses it into a standardized dataframe.
    ...
    def resample_and_generate_data(self, resample_intervals):
        ...
        flag_columns: list[SourceColumn] = [col for col in self.config.include_columns if col.name in set(column_flags)]
        # All base data columns - flag columns + indicator/function columns to be generated.
        data_columns: list[SourceColumn | IndicatorColumn] = flag_columns + self.config.indicators

        # Columns to make resampled copies of during resampling step, ordered the same way as data_columns.
        column_names_to_resample = set(col.name for col in data_columns if col.name in aggregator)
        [col for col in data_columns if col.name in column_names_to_resample]
        ...
        [col for col in columns_to_keep if col.is_scaled]

        # If data is fixed-interval:
        if type(base_interval) is timedelta:
            # Index by timestamp
            ...
            for interval in intervals:
                ...
                if interval != base_interval:
                    ...
                    scaled_columns.update(
                        self.get_col_name(col, interval)
                        for col, info in zip(ind_columns, ind_infos, strict=False)
                        if info.is_scaled
                    )
    ...
    def resample_fixed(self, df: pd.DataFrame, base_interval: timedelta, aggregator: dict, interval: str):
        """
        Resample the internal dataframe from any timestamped datatype into fixed-interval or fixed-tick columns.

        Converts any timestamped datatype (bidask, ohlc, misc)
        into fixed-interval (e.g. 5m ohlc) or fixed-tick (e.g. 9t ohlc or 9-sample aggregate)
        columns with the given intervals.
        ...
        if type(td_interval) is int:
            # Tick resampling
            # TODO: implement tick resampling
            raise ValueError("TODO: Tick resampling not supported")
        elif type(td_interval) is timedelta:
            # Timedelta resampling
            if td_interval <= base_interval:
                raise ValueError(
                    f"Failed resampling of Datasource due to interval {interval} "
                    f"less than or equal to base interval {base_interval}."
                )
    ...
    def generate_indicators(self, df: pd.DataFrame) -> tuple[pd.DataFrame, list[str], list[IndicatorColumn]]:
        """
        Generate indicators for the dataframe if OHLC and timestamp data is present. Otherwise, make no changes.
    ...
    def verify_df_interval(self, interval):
        ...
        elif type(interval) is timedelta:
            deltas = ts_col.diff()
    ...
    def verify_df_columns(self, format_flags: Semantics):
        """
        Make sure all the flagged columns are present in the given dataframe.

        Args:
            df (DataFrame): The dataframe to check.
            format_flags (DatasetColumns): The flags for this dataframe.
                OHLCV, BID/ASK, and TIMESTAMP flags require a corresponding column name in the dataframe
                (in lower case).
        ...
        if not self.check_df_columns(self._df, format_flags):
            raise ValueError(
                f"Failed verification of Datasource due to missing necessary columns "
                f"for column flags {self.config.column_flags}. Present columns: {self._df.columns}"
            )
    ...
    def check_df_columns(self, df: pd.DataFrame, format_flags: Semantics):
        ...
        return all(col in df for col in expected_columns)
