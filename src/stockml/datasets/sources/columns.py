...
class Semantics(Flag):
    ...
    def parse(cls, enum_str: str):
        """Parse a string in the format 'FLAG1 [& FLAG2] [& FLAG3] ...'."""
        ...
        for part_str in parts:
            try:
                ...
                raise ValueError(f"Invalid flag: {part_str}") from err
        return enum
...
class SourceColumn:
    ...
    def from_dict(cls, env):
        return cls(**{k: v for k, v in env.items() if k in inspect.signature(cls).parameters})  # type: ignore
...
class IndicatorColumn(SourceColumn):
    ...
    def get_indicator_name(self):
        ...
        # then {column}_
        part_a = self.params.get("column", "close")
...
def generate_indicators(df, indicators):
    ...
    def get_attrs(cls):
        return map(lambda name: (getattr(cls, name), name), dir(cls))
    ...
    for ind_json in indicators:
        ...
        if func.startswith("TA."):
            ...
            if matches is not None:
                ...
                raise ValueError(
                    "Failed indicator generation. "
                    "Indicator function began with np but does not match expected format np.func(param): "
                    + ind_conf.function
                )
        else:
