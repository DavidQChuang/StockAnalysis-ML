...
def merge(a, b, path=None):
    """Merge b into a without replacing existing values."""
    if path is None:
        path = []
    for key in b:
        if key in a:
...
def from_file(run_file: str, run_name: str, **kwargs) -> tuple[dict, str]:
    print("[###] > Reading runs from runfile (default: runs/model_runs.json or runs/sample_runs.json).")
    ...
    if os.path.exists(run_file):
        ...
        # Get global run data
        global_run_data = file_json.get("global", {})
...
def from_input(run_file: str, **kwargs) -> tuple[dict, str]:
    print("[###] > Reading run from runfile (default: runs/model_runs.json or runs/sample_runs.json).")
