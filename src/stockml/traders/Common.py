...
from stockml.traders.dqn.LinearDQN import LinearDQN
from stockml.traders.TradingDataset import TradingDataset
from stockml.traders.TradingSimulation import TradingSimulation
...
class StandardTrader:
    ...
    def get_state(self, trade_sim: TradingSimulation, datapoint, device):
        """
        Return the current state of the DQN.
    ...
    def standard_train(self, model: StandardModel, dataset: TimeSeriesDataset):
        ...
        if self.policy_net_state is not None:
            policy_net.load_state_dict(self.policy_net_state)
        if self.target_net_state is not None:
        ...
        if first_run:
            self.runtime = {"episode": 0, "steps": 0}
        ...
        for e in range(conf.episodes):
            ...
            idx_range = range(batch * conf.batch_size, min((batch + 1) * conf.batch_size, len(scaled_dataset)) - 1)
            ...
            for idx in train_progress:
                ...
                for key in policy_net_state_dict:
                    target_net_state_dict[key] = policy_net_state_dict[key] * conf.tau + target_net_state_dict[key] * (
                        1 - conf.tau
                    )
            ...
            print(
                f"avg_profit diff: {(avg_profit - avg_mkt_profit) / iters:.2f}$; "
                f"avg_delta diff: {(avg_delta - avg_mkt_delta) / iters:.2f}$"
            )
            print(
                f"very bad(+/-): {verybad}, bad(-/-): {bad}, good(+/+): {good},"
                f" goodish(-<-): {goodish}, very good(-/+): {verygood}"
            )
    ...
    def load(self, filename=None):
        ...
        self.runtime = {"episode": ckpt["episode"], "steps": ckpt["steps"]}
