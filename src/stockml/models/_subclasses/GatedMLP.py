...
import stockml.models.embeddings as embeddings
from stockml.models.Common import ModelConfig
...
class SpatialGatingUnit(nn.Module):
    def __init__(self, d_ffn, seq_len, init_eps=1e-3):
        ...
        Args:
            ...
            d_ffn (int): dimensionality of the normalization layer
            init_eps (float): magnitude of initialization epsilons for the weights
...
class GatedMLPBlock(nn.Module):
    def __init__(self, d_ffn, d_model, seq_len, d_attn=0, activation=None):
        ...
        Args:
            ...
            seq_len (int): length of the token sequence; window size (n)
            d_attn (int): dimensionality of the self-attention part
...
class GatedMLP(nn.Module):
    @classmethod
    def get_config(cls, model_json):
        if model_json is not None and "gmlp" not in model_json:
            raise Exception("'gmlp' key must be present in model.gmlp parameters.")

        return GatedMLPConfig.from_dict(model_json["gmlp"])

    def __init__(self, conf: ModelConfig, gmlp: GatedMLPConfig):
        super().__init__()
        ...
        d_model = feature_count if embedding_length == 0 else embedding_length * feature_count
        ...
        self.linear = nn.Sequential(
            nn.ReLU(inplace=True), nn.Linear(d_ffn, d_ffn), nn.ReLU(inplace=True), nn.Linear(d_ffn, 1)
        )
    ...
    def legacy_offset(self, x):
        ...
        if len(x.shape) == 1:
            x = x[None, :, None]
        if len(x.shape) == 2:
            # This unsqueezes x from (n, f) to (1, n, f) if f == feature_count
            # else unsqueezes x from (b, n) to (b, n, 1)
            x = x.unsqueeze(0) if x.shape[1] == self.feature_count else x.unsqueeze(-1)
