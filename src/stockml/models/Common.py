...
class PytorchModel(StandardModel):
    ...
    def __init__(self, network: nn.Module, columns: list[str], scaled_columns: list[str], model_json: dict, device=None, verbosity=1):
    ...
    def load(self, filename=None):
        ...
        if 'mean_' in ckpt:
            ...
            if not 'columns' in ckpt:
                ...
                raise ValueError(f"Model is invalid; Columns are different from saved model. Old: {self.runtime['columns']}, New: {self.columns}.")
            elif self.runtime['scaled_columns'] != self.scaled_columns:
                raise ValueError(f"Model is invalid; Scaled columns are different from saved model. Old: {self.runtime['scaled_columns']}, New: {self.scaled_columns}.")
...
class DeepspeedModel(PytorchModel):
    def __init__(self, module: nn.Module, columns: list[str], scaled_columns: list[str], model_json, device=None, verbosity=1):
        ...
        model_engine, optimizer, _, _ = deepspeed.initialize(config=model_json['deepspeed'],
                                                     ...
                                                     model_parameters=self.module.parameters()) # type: ignore
