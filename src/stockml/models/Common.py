...
import stockml.models.loss as loss
...
def get_bar_format(dataset_len, batch_size):
    ...
    bar_format = "{n_fmt:>%d}/{total_fmt:%d} [{bar:30}] {elapsed} - eta: {remaining}, {rate_fmt}{postfix}" % (
        len_n_fmt,
        len_n_fmt,
    )
    return bar_format
...
class ModelConfig:
    ...
    def model_filename(self):
        return "%d_%d+%d" % (self.hidden_layer_size, self.seq_len, self.out_seq_len)


class StandardModel(ABC):
    def __init__(self, model_json, columns: list[str], scaled_columns: list[str], device=None, verbosity=1):
        super().__init__()
    ...
    def get_filename(self):
        ...
        return f"{classname}-{filename}f{columns}.ckpt"

    @property
    def pin_memory(self):
        return self.device is not None and not self.device.startswith("cpu") and self.conf.pin_memory
...
class PytorchModel(StandardModel):
    ...
    def __init__(
        ...
        super().__init__(model_json, columns, scaled_columns, device, verbosity)
        ...
        if device is not None and not device.startswith("cpu"):
    ...
    def get_optimizer(self):
        optimizer = optim.Adam(
            self.module.parameters(), lr=self.conf.learning_rate, weight_decay=self.conf.weight_decay
        )
    ...
    def single_train(self, X, Y, loss_func, optimizer):
        """
        Perform a single forward and backward step with the optimizer and a loss calculation.
    ...
    def single_infer(self, X, Y, loss_func):
        """
        Perform a single inference and loss calculation with gradients off.
    ...
    def infer(self, X, scale_inputs=True, scale_outputs=True):
        with torch.no_grad():
            input = self.scale_input(X) if scale_inputs else X
    ...
    def standard_train(self, dataset: TimeSeriesDataset, iter_callback=None, data_callback=None, epoch_callback=None):
        ...
        # -- Setup
        use_cuda = self.device is not None and self.device != "cpu"
        ...
        first_run = self.runtime is None
        if self.runtime is None:
            self.runtime = {"epoch": 0, "loss": 0, "val_loss": 0}

        # Scale data. If the scaler hasn't been fit, the below will be true.
        should_fit_data = not hasattr(self.scaler, "mean_")
        # If scaler hasn't been fit, this will fit and transform. Else, it will only transform.
        dataset = dataset.scale_dataset(self.scaler, should_fit_data)  # dataset still numpy
        if should_fit_data:
            self.runtime["columns"] = self.columns
            self.runtime["scaled_columns"] = self.scaled_columns
        ...
        for e in range(run_epochs):
            ...
            for train_iter, data in enumerate(train_progress):
                ...
                train_err_max = max(
                    train_err_max, self.scale_output(err_vec.max().item(), column=dataset.target, is_delta=True)
                )
                ...
                train_loss / (train_iter + 1)
                disp_acc = train_acc / (train_iter + 1)
                disp_err = self.scale_output(
                    train_err / (train_iter + 1), column=dataset.target, is_delta=True
                )  # accurate if loss < 1
        ...
        self.runtime["loss"] = train_loss / train_iter  # type: ignore
        self.runtime["val_loss"] = valid_loss / valid_iter  # type: ignore
        self.optimizer_state = None if optimizer is None else optimizer.state_dict()

    def save(self, filename=None):
        """
        Save the model to a checkpoint file. If no filename is given, defaults to ckpt/{get_filename()}.

        The model must be initialized.
        Models can be initialized by one of two methods:
            loading from file with self.load(),
            or training a new model with self.standard_train(dataset).

        Args:
            filename (str, optional): The path to save the checkpoint file to. \
                                      Defaults to ckpt/{get_filename()}.

        Raises:
            RuntimeError: If there is no runtime data from an initialized model (see above), raises a RuntimeError.

        """
        filename = filename if filename is not None else f"ckpt/{self.get_filename()}"
        ...
        ckpt = {
            ...
            "scaled_columns": self.runtime["scaled_columns"],
            "mean_": self.runtime["mean_"],
            "var_": self.runtime["var_"],
            "scale_": self.runtime["scale_"],
        }

        print({"epoch": ckpt["epoch"], "loss": ckpt["loss"], "val_loss": ckpt["val_loss"]})
        print()
    ...
    def load(self, filename=None):
        ...
        if "mean_" in ckpt:
            self.runtime = {
                ...
                "scaled_columns": ckpt["scaled_columns"],
                "mean_": ckpt["mean_"],
                "var_": ckpt["var_"],
                "scale_": ckpt["scale_"],
            }

            if "columns" not in ckpt:
                print("> !!! Model has no columns, saved model may not match.")
            elif self.runtime["columns"] != self.columns:
                raise ValueError(
                    f"Model is invalid; Columns are different from saved model. "
                    f"Old: {self.runtime['columns']}, New: {self.columns}."
                )
            elif self.runtime["scaled_columns"] != self.scaled_columns:
                raise ValueError(
                    f"Model is invalid; Scaled columns are different from saved model. "
                    f"Old: {self.runtime['scaled_columns']}, New: {self.scaled_columns}."
                )
    ...
    def print_debug_nan(self, X, Y, optimizer):
        ...
        grad_flat = [torch.flatten(weight.grad) for weight in weights if weight.grad is not None]
        if grad_flat != []:
...
class DeepspeedModel(PytorchModel):
    def __init__(
        self, module: nn.Module, columns: list[str], scaled_columns: list[str], model_json, device=None, verbosity=1
    ):
        ...
        model_engine, optimizer, _, _ = deepspeed.initialize(
            config=model_json["deepspeed"],
            model=self.module,  # type: ignore
