...
2: diagnostic - the above + model summary, 
    ...
    if args.use_visualizer:
        from stockml.visualizer import run_app
...
def main(args: argparse.Namespace, worker=None, app=None):
    ...
    vprint(verbosity, 1, "├[1/3] > Loading dataset.")
    dataset = stockml.datasets.from_run(run_data, **args_dict)
    vprint(verbosity, 1, "├[2/3] > Loading model.")
    ...
    if args.detect_anomaly:
        anomaly.set_detect_anomaly(True)

    # Run mode
    if args.eval_inference_count is None:  # and args.eval_trader_count == None:
        trader = stockml.traders.from_run(run_data, **args_dict)
        print()

        if not os.path.isdir("ckpt"):
            os.mkdir("ckpt")

        if model.conf.epochs != 0:
            model_file = load_thing(model, args.model_file, args.rebuild_model)
            model.standard_train(
                dataset,
                iter_callback=None if worker is None else worker.iter_callback(),
                data_callback=None if worker is None else worker.data_callback(),
            )  # type: ignore
        ...
        if "timestamp" in dt_cols:
            ...
            raise KeyError(
                "No 'timestamp' column with np.datetime64 dtype.  "
                "If datasets do not include such a column it is not being loaded properly."
            )

        print(
            f"# [timestep] Detected timestep ({timestep.days} days  "
            f"{timestep.seconds // 3600:02d}:{timestep.seconds // 60 % 60:02d}:{timestep.seconds % 60:02d})."
        )

        if timestep <= timedelta(minutes=0):
            print("!! timestep: ", timestep)
            raise ValueError(
                "Detected timestep was non-positive. The dataset is in the wrong order or has duplicate elements."
            )
        ...
        for i in range(-args.eval_inference_count, 1):
            ...
            if i != 0:
                print(
                    f"# [realvalue] [step={i}], Date: [date={curr_date.strftime(strftime_format)}], "
                    f"Value($): [val={dataset.df['close'].iloc[i]}]"
                )
            print(
                f"# [inference] [step={i}], Date: [date={curr_date.strftime(strftime_format)}], "
                f"Value($): [val={output:.2f}]"
            )
            curr_date += timestep


def load_thing(model, model_file, rebuild_model):
    model_file = f"ckpt/{model.get_filename()}" if model_file is None else model_file
